...
class Template(object):
	def __init__(self, args):
		...
		self.sub_punto = rospy.Subscriber('/duckietown/camera_node/punto' , Point, self.emergencia)
	...
	def callback(self,msg):
		if msg.buttons[1]==1:
			...
			self.twist.omega = 0
		#Patito atropellado
#		elif point.z <= 10 or abs(point.x) <= 8:
#			rospy.loginfo("Emergencia camara")
#			self.twist.v = 0
#			self.twist.omega = 0
		else:
	...
	def emergencia(self,point):
		if point.z <= 10 or abs(point.x) <= 8:
			rospy.loginfo("Emergencia camara")
			self.twist.v = 0
			self.twist.omega = 0
			print "if"
